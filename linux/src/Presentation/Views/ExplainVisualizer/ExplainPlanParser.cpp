...
namespace gridex::explainviz
{
    ...
    namespace
    {
        struct Reader
        {
            ...
            // Hand-rolled JSON number parser. Locale-independent — unlike
            // std::wcstod, which honors LC_NUMERIC and breaks on locales
            // (e.g. vi_VN, de_DE) where the decimal separator is ',' not
            // '.'. QApplication switches the process locale on startup,
            // so the C-library parser misreads "0.00" as just "0" and the
            // Plan tree falls apart mid-parse.
            bool parseNumber(double& out)
            {
                skipWs();
                const wchar_t* start = p;
                bool neg = false;
                if (p < end && *p == L'-') { neg = true; ++p; }
                else if (p < end && *p == L'+') ++p;

                bool seenDigit = false;
                double intPart = 0.0;
                while (p < end && *p >= L'0' && *p <= L'9') {
                    intPart = intPart * 10.0 + double(*p - L'0');
                    ++p; seenDigit = true;
                }
                ...
                if (p < end && *p == L'.') {
                    ++p;
                    while (p < end && *p >= L'0' && *p <= L'9') {
                        fracPart = fracPart * 10.0 + double(*p - L'0');
                        fracScale *= 10.0;
                        ++p; seenDigit = true;
                    }
                }
                ...
                if (p < end && (*p == L'e' || *p == L'E')) {
                    ++p;
                    if (p < end && *p == L'-') { expSign = -1; ++p; }
                    else if (p < end && *p == L'+') ++p;
                    while (p < end && *p >= L'0' && *p <= L'9') {
                        expVal = expVal * 10 + int(*p - L'0');
                        ++p;
                    }
                }

                if (!seenDigit) { p = start; return false; }
                double v = intPart + fracPart / fracScale;
                if (neg) v = -v;
                if (expVal != 0) {
                    double m = 1.0;
                    for (int i = 0; i < expVal; ++i) m *= 10.0;
                    v = (expSign > 0) ? v * m : v / m;
                }
                out = v;
                return true;
